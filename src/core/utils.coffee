module.exports =
  ...
  toTitleCase : (str) ->
    ...
    str.replace /[A-Za-z0-9\u00C0-\u00FF]+[^\s-]*/g, (match, index, title) ->
      return match.toLowerCase() if index > 0 and index + match.length isnt title.length \
      and match.search(smallWords) > -1 and title.charAt(index - 2) isnt ":" \
      and (title.charAt(index + match.length) isnt "-" or title.charAt(index - 1) is "-") \
      and title.charAt(index - 1).search(/[^\s-]/) < 0
